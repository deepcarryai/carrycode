...
export interface WelcomeModelWizardProps {
  sessionId: string;
  onFinished: () => void;
  onCancelled: () => void;
}
...
function WizardProgressBar(props: {
  ...
  const truncateLabelToWidth = (text: string, maxWidth: number) => {
  ...
  const trimLabel = (label: string) => {
    ...
    if (segmentWidth <= 1) return truncateLabelToWidth(label, 1);
    return truncateLabelToWidth(label, segmentWidth - 1) + '…';
  };
...
function ReviewStepContent(props: {
  ...
  const { t } = useTranslation();
  const { ctx, providerName, providerBrand, baseUrl, apiKey, modelName, submitting, onSubmit, onTitle, onHint } = props;
...
export function WelcomeModelWizard({ sessionId, onFinished, onCancelled }: WelcomeModelWizardProps) {
  const { t } = useTranslation();
  ...
  const { listProviderPresets, saveUserProviders, reloadSessionConfig, setModel } = useRustBridge();
  ...
  const [stepTitle, setStepTitle] = useState<string>(() => t('welcome_wizard.titles.provider_brand'));
  const [stepSubtitle, setStepSubtitle] = useState<string>('');

  const getPreset = (brand: string): ProviderPreset | undefined => presets.find((p) => p.providerId === brand);

  const applyPreset = (brand: string) => {
  ...
  const persistProvider = async () => {
    ...
    try {
      logger.info(
        `welcomeModelWizard.submit start providerBrand=${providerBrand} providerName=${providerName} baseUrl=${baseUrl} modelName=${modelName} apiKey=${maskSecret(
          apiKey,
        )}`,
      );
      ...
      logger.error(`welcomeModelWizard.submit failed: ${msg}`);
      setErrorText(msg);
      setSubmitting(false);
    }
  };
  ...
  }, [errorText, hintText, theme.colors.warning]);
