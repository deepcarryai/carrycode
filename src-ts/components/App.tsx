...
export function App() {
  ...
  const { askAgent, createSessionId, confirmTool, getSessionHistory, getAgentMode } = useRustBridge();
  ...
  const [configMenuInitialLevel, setConfigMenuInitialLevel] = useState<'main' | 'model' | 'theme'>('main');
  ...
  async function switchSession(targetSessionId: string) {
    ...
    try {
      try {
        const mode = getAgentMode(targetSessionId);
  ...
  async function handleSubmit(input: string) {
    ...
    if (input.trim() === '/config') {
      setConfigMenuInitialLevel('main');
      setShowConfigMenu(true);
      return;
    }

    if (input.trim() === '/themes') {
      setConfigMenuInitialLevel('theme');
      setShowConfigMenu(true);
      return;
    }

    if (input.trim() === '/model') {
      setConfigMenuInitialLevel('model');
      setShowConfigMenu(true);
      return;
    }
    ...
    if (trimmed.startsWith('/sessions ')) {
      const target = trimmed.replace(/^\/sessions\s+/, '').trim();
