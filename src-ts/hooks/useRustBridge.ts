...
export function useRustBridge() {
  function listAvailableSkills(sessionId: string) {
    return coreapi.listAvailableSkills(sessionId);
  }

  function getSkillMarkdown(sessionId: string, skillName: string) {
    return coreapi.getSkillMarkdown(sessionId, skillName);
  }

  async function enableSkillForSession(sessionId: string, skillName: string) {
    return coreapi.enableSkillForSession(sessionId, skillName);
  }

  async function disableSkillForSession(sessionId: string, skillName: string) {
    return coreapi.disableSkillForSession(sessionId, skillName);
  }
  ...
  function listProviderPresets(): ProviderPreset[] {
    return coreapi.listProviderPresets();
  }

  function getConfigBootstrapState(): ConfigBootstrapState {
    return coreapi.getConfigBootstrapState();
  }

  async function setLanguage(language: string): Promise<void> {
    return coreapi.setLanguage(language);
  }

  async function setWelcomeWizardDone(done: boolean): Promise<void> {
    return coreapi.setWelcomeWizardDone(done);
  }

  async function saveUserProviders(
    providers: Array<{
      providerBrand: string;
      providerId: string;
      modelName: string;
    ...
    return coreapi.saveUserProviders(providers);
  }

  async function getSession(sessionId: string) {
    let session = sessionInstances.get(sessionId);
    if (!session) {
        session = await coreapi.Session.open(sessionId);
  ...
  async function reloadSessionConfig(sessionId: string): Promise<void> {
      const session = await getSession(sessionId);
  ...
  async function getAgentMode(sessionId: string): Promise<'plan' | 'build'> {
      const session = await getSession(sessionId);
  ...
  async function getApprovalMode(sessionId: string): Promise<'read-only' | 'agent' | 'agent-full'> {
      const session = await getSession(sessionId);
