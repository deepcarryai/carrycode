...
impl EditTool {
    ...
    pub fn new_with_lsp(lsp_manager: Option<Arc<LspManager>>) -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.tool_edit.tool_name,
                description: config.tool_edit.description,
                lsp_manager,
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.tool_edit.tool_name.clone(),
            description: config.tool_edit.description.clone(),
            lsp_manager: None,
            lsp_timeout_ms: config.lsp.timeout_ms,
        }
    }
    ...
    fn run_edit(&self, request: &EditRequest) -> Result<EditResult> {
        ...
        let (diff_str, additions, removals) = Self::calculate_diff(&original_content, &new_content);
    ...
    fn calculate_diff(old_content: &str, new_content: &str) -> (String, usize, usize) {
        ...
        (diff_output, additions, removals)
    }
