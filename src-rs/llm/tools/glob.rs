...
impl GlobTool {
    ...
    pub fn new() -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.tool_glob.tool_name,
                description: config.tool_glob.description,
                max_glob_results: config.tool_glob.max_glob_results,
            },
            Err(e) => {
                log::warn!(
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.tool_glob.tool_name.clone(),
            description: config.tool_glob.description.clone(),
            max_glob_results: config.tool_glob.max_glob_results,
        }
    }
    ...
    pub fn run_glob(&self, request: &GlobRequest) -> Result<GlobResult> {
    ...
    fn try_ripgrep(&self, request: &GlobRequest) -> Result<GlobResult> {
    ...
    fn run_glob_native(&self, request: &GlobRequest) -> Result<GlobResult> {
        let policy = PathPolicy::new()?;
        let base_path = request
            .path
            .as_deref()
...
impl ToolSpec for GlobTool {
    type Args = GlobRequest;
