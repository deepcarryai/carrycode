...
impl ViewTool {
    ...
    pub fn new() -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.tool_view.tool_name,
                description: config.tool_view.description,
                max_file_size: config.tool_view.max_file_size,
            },
            Err(e) => {
                log::warn!(
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.tool_view.tool_name.clone(),
            description: config.tool_view.description.clone(),
            max_file_size: config.tool_view.max_file_size,
        }
    }
    ...
    fn run_view(&self, request: &ViewRequest) -> Result<ViewResult> {
        ...
        // Generate preview
        let preview = truncate_utf8_with_ellipsis(&first_line_for_preview, 80);
...
impl Default for ViewTool {
    fn default() -> Self {
        Self {
            ...
            description:
                "File viewing tool that reads and displays the contents of files with line numbers."
                    .to_string(),
...
impl ToolSpec for ViewTool {
    type Args = ViewRequest;
