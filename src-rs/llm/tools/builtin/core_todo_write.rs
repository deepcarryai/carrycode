...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CoreTodoWriteRequest {
    #[serde(deserialize_with = "deserialize_vec_or_str_lax")]
    pub todos: Vec<TodoItem>,
}

/// Result of todo write operation
impl CoreTodoWriteTool {
    pub fn new() -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.core_todo_write.tool_name,
                description: config.core_todo_write.description,
            },
            Err(e) => {
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.core_todo_write.tool_name.clone(),
            description: config.core_todo_write.description.clone(),
        }
    }
}

impl Default for CoreTodoWriteTool {
    fn default() -> Self {
        Self {
            tool_name: "core_todo_write".to_string(),
            description: "[CORE SYSTEM] Manage task lists and track progress".to_string(),
        }
    }
}

impl ToolSpec for CoreTodoWriteTool {
    type Args = CoreTodoWriteRequest;
