...
impl CoreGrepTool {
    ...
    pub fn new() -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.core_grep.tool_name,
                description: config.core_grep.description,
                max_grep_results: config.core_grep.max_grep_results,
            },
            Err(e) => {
                log::warn!(
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.core_grep.tool_name.clone(),
            description: config.core_grep.description.clone(),
            max_grep_results: config.core_grep.max_grep_results,
        }
    }
    ...
    pub fn run_grep(&self, request: &CoreGrepRequest) -> Result<GrepResult> {
    ...
    fn try_ripgrep(&self, request: &CoreGrepRequest) -> Result<GrepResult> {
    ...
    fn run_grep_native(&self, request: &CoreGrepRequest) -> Result<GrepResult> {
        tokio::task::block_in_place(|| self.run_grep_native_inner(request))
    }

    fn run_grep_native_inner(&self, request: &CoreGrepRequest) -> Result<GrepResult> {
        let base_path = request
            .path
            .as_ref()
...
impl Default for CoreGrepTool {
    fn default() -> Self {
        Self {
            tool_name: "core_grep".to_string(),
            description:
                "[CORE SYSTEM] Search file contents for text patterns. Returns matching file paths and contexts."
                    .to_string(),
...
impl ToolSpec for CoreGrepTool {
    type Args = CoreGrepRequest;
