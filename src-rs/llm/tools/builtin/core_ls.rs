...
impl CoreLsTool {
    ...
    pub fn new() -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.core_ls.tool_name,
                description: config.core_ls.description,
                max_ls_files: config.core_ls.max_ls_files,
                default_ignore: config.core_ls.default_ignore,
            },
            Err(e) => {
                log::warn!(
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.core_ls.tool_name.clone(),
            description: config.core_ls.description.clone(),
            max_ls_files: config.core_ls.max_ls_files,
            default_ignore: config.core_ls.default_ignore.clone(),
        }
    }
...
impl Default for CoreLsTool {
    fn default() -> Self {
        Self {
            tool_name: "core_ls".to_string(),
            description: "[CORE SYSTEM] List directory contents in tree structure. Shows files and directories with automatic filtering of hidden files and common ignore directories.".to_string(),
            max_ls_files: 1000,
            default_ignore: vec![
...
impl ToolSpec for CoreLsTool {
    type Args = CoreLsArgs;
