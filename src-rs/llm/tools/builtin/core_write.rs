...
impl CoreWriteTool {
    ...
    pub fn new_with_lsp(lsp_manager: Option<Arc<LspManager>>) -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.core_write.tool_name,
                description: config.core_write.description,
                lsp_manager,
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.core_write.tool_name.clone(),
            description: config.core_write.description.clone(),
            lsp_manager: None,
            lsp_timeout_ms: config.lsp.timeout_ms,
        }
    }
    ...
    pub(crate) fn run_write(&self, request: &CoreWriteRequest) -> Result<WriteResult> {
        ...
        if file_exists {
            ...
            if !append && original_content == request.content {
            ...
            let final_content = if append {
                format!("{}{}", original_content, request.content)
            } else {
                request.content.clone()
            };
            let diff = Self::calculate_diff(&original_content, &final_content);

            // Calculate additions and removals
            let (add, rem) = Self::count_changes(&original_content, &final_content);
            additions = add;
            removals = rem;
        ...
        if append && file_exists {
            ...
            f.write_all(request.content.as_bytes())
        ...
        let success_message = if file_exists {
            if append {
                format!("Successfully appended to file: {}", request.file_path)
            } else {
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "content": {
                            "type": "string",
                            "description": "The content to write to the file. If the JSON contains newlines in this string value, they must be escaped as \\n. For very large files, you may split into multiple core_write calls using append=true to avoid model truncation."
                        },
                        "append": {
                            "type": "boolean",
                            "description": "If true, append content to the end of the file instead of overwriting. Defaults to false. Use this for chunked writes."
                        }
                    },
...
impl Default for CoreWriteTool {
    fn default() -> Self {
        Self {
            tool_name: "core_write".to_string(),
            description: "[CORE SYSTEM] File writing tool that creates or updates files in the filesystem."
                .to_string(),
            lsp_manager: None,
            lsp_timeout_ms: 5000,
        }
...
impl ToolSpec for CoreWriteTool {
    type Args = CoreWriteRequest;
    ...
    fn run(&self, args: Self::Args, _confirmed: bool) -> Result<ToolResult> {
        ...
        let content_chars = args.content.chars().count();
        if content_chars > limit {
            anyhow::bail!(
                "core_write content too long ({} chars; limit {}). Split into multiple core_write calls: first append=false, then append=true.",
                content_chars,
                limit
            );
        }
