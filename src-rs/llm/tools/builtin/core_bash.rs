...
impl PersistentShell {
    ...
    let stdout_file = temp_dir.join(format!("carry-stdout-{}", timestamp));
    let stderr_file = temp_dir.join(format!("carry-stderr-{}", timestamp));
    let status_file = temp_dir.join(format!("carry-status-{}", timestamp));
...
impl CoreBashTool {
    pub fn new() -> Self {
        match AppConfig::load() {
            Ok(config) => Self {
                tool_name: config.core_bash.tool_name,
                description: config.core_bash.description,
                banned_commands: config.core_bash.banned_commands,
                safe_read_only_commands: config.core_bash.safe_read_only_commands,
            },
            Err(_) => Self::default(),
        }
    }
    ...
    pub fn from_config(config: &AppConfig) -> Self {
        Self {
            tool_name: config.core_bash.tool_name.clone(),
            description: config.core_bash.description.clone(),
            banned_commands: config.core_bash.banned_commands.clone(),
            safe_read_only_commands: config.core_bash.safe_read_only_commands.clone(),
        }
    }
    ...
    pub fn run_bash(&self, request: &CoreBashRequest) -> Result<BashResult> {
    ...
    fn execute_command(&self, request: &CoreBashRequest, _confirmed: bool) -> Result<BashResult> {
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "command": {
                            "type": "string",
                            "description": "The bash command to execute. If the JSON contains newlines in this string value, they must be escaped as \\n. For very large text writes, prefer chunked core_write calls with append=true."
                        },
                        "workdir": {
                            "type": "string",
...
use crate::llm::utils::string_util::truncate_middle;

pub fn truncate_output(content: &str) -> String {
    truncate_middle(content, MAX_OUTPUT_LENGTH)
}

impl Default for CoreBashTool {
    fn default() -> Self {
        Self {
            tool_name: "core_bash".to_string(),
            description: "[CORE SYSTEM] Executes a given bash command in a persistent shell session.".to_string(),
            banned_commands: vec![
                "alias".to_string(),
                "curl".to_string(),
                "curlie".to_string(),
...
impl ToolSpec for CoreBashTool {
    type Args = CoreBashRequest;
    ...
    fn run(&self, mut args: Self::Args, confirmed: bool) -> Result<ToolResult> {
        ...
        let model_key = cfg
            .as_ref()
            .and_then(|c| c.runtime.default_model.as_deref().or(c.default_model.as_deref()));
        let limit = cfg
            .as_ref()
            .map(|c| c.tool_chunking.limit_chars_for_model(model_key))
            .unwrap_or(1000);

        let command_chars = args.command.chars().count();
        if command_chars > limit {
            anyhow::bail!(
                "core_bash command too long ({} chars; limit {}). Split into multiple smaller commands, or prefer chunked core_write calls with append=true.",
                command_chars,
                limit
            );
        }
