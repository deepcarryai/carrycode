...
use crate::llm::tools::builtin::core_tool_base::{
...
fn tool_result_from_execution(
    ...
    let key_path = key_path_from_args(tool_name, args, None, None);
...
impl Agent {
    ...
    pub fn set_model(&mut self, provider_name: &str, model_name: &str) -> Result<()> {
        ...
        for c in &self.provider_configs {
        ...
        if found.is_none() {
            if self.provider_configs.iter().any(|c| c.name == provider_name) {
                return Err(anyhow::anyhow!(
    ...
    pub fn get_base_url(&self) -> String {
        self.provider_configs
            .iter()
            .find(|c| c.name == self.provider_name && c.models.iter().any(|m| m == &self.model_name))
    ...
    pub fn add_assistant_message(&mut self, content: String, reasoning_content: Option<String>) {
        self.messages.push(Message {
    ...
    pub async fn execute(&mut self) -> Result<AgentResult> {
        ...
        loop {
            ...
            if !current_content.is_empty() || tool_calls_json_str.is_some() || !current_reasoning.is_empty() {
                let mut full_content = current_content.clone();
                if let Some(json_str) = &tool_calls_json_str {
                    if !full_content.is_empty() {
                        full_content.push_str("\n\n");
                    }
                    full_content.push_str(&format!("<agent_tool_calls_json>\n{}\n</agent_tool_calls_json>", json_str));
                }
                
                let reasoning_opt = if current_reasoning.is_empty() {
                    None
                } else {
                    Some(current_reasoning)
                };
                
                self.add_assistant_message(full_content, reasoning_opt);
            ...
            if !tool_calls_map.is_empty() {
                ...
                for (_index, (tool_call_id_opt, tool_name_opt, arguments_acc)) in tool_calls_map {
                    ...
                    match &*self.client {
                        ...
                        _ => {
                            let mut res_val: Value = serde_json::from_str(&tool_result_json)
                                .unwrap_or_else(|_| json!({ "raw": tool_result_json }));
                            if let Some(id) = tool_call_id_opt {
                                if let Some(obj) = res_val.as_object_mut() {
                                    obj.insert("tool_call_id".to_string(), json!(id));
                                }
                            }
                            let final_json = serde_json::to_string(&res_val).unwrap_or(tool_result_json);
                            self.add_user_message(format!("ToolResult:\n{}", final_json));
                        }
                    }
                }
