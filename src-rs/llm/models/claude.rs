    ...
    Box::pin(
        async_stream::stream! {
            ...
            while let Some(chunk_result) = tokio_stream::StreamExt::next(&mut stream).await {
                ...
                sse_chunk_count += 1;
                log::trace!(
                    "SSE parser received chunk #{}: {} bytes",
                    sse_chunk_count,
                    bytes.as_ref().len()
                );
                ...
                while let Some(frame_bytes) = extract_sse_frame_from_buffer(&mut buffer) {
                    ...
                    if let Some(data) = sse_data_from_frame(&frame) {
                        emitted_count += 1;
                        log::debug!(
                            "SSE emitting frame event #{}: {} chars",
                            emitted_count,
                            data.len()
                        );
                        yield Ok(data);
            ...
            log::debug!(
                "SSE stream ended. Remaining buffer: {} bytes, line_acc: {} chars",
                buffer.len(),
                line_acc.len()
            );
            ...
            if !buffer.is_empty() {
                ...
                if let Some(data) = sse_data_from_frame(&frame) {
                    emitted_count += 1;
                    log::debug!(
                        "SSE emitting final frame event #{}: {} chars",
                        emitted_count,
                        data.len()
                    );
                    yield Ok(data);
                    ...
                    if !frame_trimmed.is_empty() {
                        emitted_count += 1;
                        log::debug!(
                            "SSE emitting raw response as event #{}: {} chars",
                            emitted_count,
                            frame_trimmed.len()
                        );
            ...
            if !line_acc.is_empty() {
                emitted_count += 1;
                log::debug!(
                    "SSE emitting final line_acc event #{}: {} chars",
                    emitted_count,
                    line_acc.len()
                );
                yield Ok(line_acc);
    ...
    async fn stream_chat(
        ...
        for msg in messages {
            if msg.role == "system" {
                ...
                if
                    let Some((tool_use_id, result)) = tool_result_block_from_message_content(
                        &msg.content
                    )
                {
                    let block =
                    ...
                    if let Some((text, calls)) = extract_tool_calls_from_content(&msg.content) {
                        ...
                        for call in calls {
                            let args_str = call
                                ...
                                .and_then(|v| v.as_str())
                                .unwrap_or("{}");
                            let input_obj: Value = serde_json
                                ::from_str(args_str)
                                .unwrap_or(json!({}));
        ...
        let response = client
            ...
            .send().await
            .context(
                "Failed to send request to Anthropic API (possible timeout or network error)"
            )?;
...
mod tests {
    ...
    fn openai_tool_to_anthropic_converts_schema() {
        ...
        assert_eq!(
            anthropic_tool.get("name").and_then(|v| v.as_str()),
            Some("grep")
        );
