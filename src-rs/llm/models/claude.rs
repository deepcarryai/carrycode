...
pub(crate) fn openai_tool_to_anthropic(tool: &Value) -> Option<Value> {
...
pub(crate) fn tool_result_block_from_message_content(content: &str) -> Option<(String, Value)> {
...
fn extract_tool_calls_from_content(content: &str) -> Option<(String, Vec<Value>)> {
    ...
    if let Some(start_pos) = content.find(start_tag) {
        let text_before = content[..start_pos].trim();
        let rest = &content[start_pos + start_tag.len()..];
        ...
        if let Ok(calls) = serde_json::from_str::<Vec<Value>>(json_str.trim()) {
            let combined_text = if !text_before.is_empty() && !text_after.is_empty() {
            ...
            return Some((combined_text, calls));
        }
    }
    None
}
...
pub(crate) fn unwrap_event_data(v: &Value) -> Option<Value> {
