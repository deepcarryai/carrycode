...
impl Session {
    #[napi(factory)]
    pub async fn open(session_id: String) -> Result<Self> {
        let parts = session_util::open_session(session_id).await?;
        Ok(Self {
            inner: parts.inner,
    ...
    pub async fn cancel(&self) -> Result<()> {
        session_util::cancel_session(&self.session_id, &self.confirmation_sender).await
    }
    ...
    pub async fn set_model(&self, provider: String, model: String) -> Result<()> {
        session_util::set_model(&self.session_id, &self.inner, provider, model).await
    }

    #[napi]
    pub async fn reload_config(&self) -> Result<()> {
        session_util::reload_config(&self.inner).await
    }

    #[napi]
    pub async fn check_latency(&self) -> Result<LatencyInfo> {
        let info = session_util::check_latency(&self.session_id).await?;
        Ok(LatencyInfo {
